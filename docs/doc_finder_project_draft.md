# 프로젝트 초안 문서
## 대량 일러스트 이미지 태그 생성 및 검색 인덱스 시스템 (Working Title: DocFinder)

- 문서 버전: v0.2 Draft
- 문서 성격: 프로젝트 개요 + 제품 요구사항 + 기술 설계 초안
- 기준: 현재 대화에서 합의된 방향을 반영한 1차 재작성본
...
1. 프로젝트의 1차 목표를 `이미지 태그 생성 및 저장` 중심으로 명확히 정의한다.
2. CMS 원본 데이터와 DocFinder 인덱스의 역할을 분리한다.
3. 일러스트 이미지 태깅 파이프라인과 검색 인터페이스를 구현 가능한 수준으로 정리한다.
4. LangGraph를 태깅 파이프라인 오케스트레이션 레이어로 사용하는 이유를 명확히 한다.
...
**10만 개 이상의 일러스트 이미지에 구조화된 태그를 자동 생성·저장하고, 객체명 검색과 필터링이 가능하도록 인덱스를 구축하는 시스템을 만든다.**

핵심은 단순 저장이 아니다. 다음 네 가지를 한 번에 해결하는 것이다.
...
- `사과`, `고양이`, `버스` 같은 객체명으로 일러스트를 찾고 싶다.
...
이 프로젝트는 단순 CMS 보조 기능이 아니다. **일러스트 이미지용 태그 인덱스와 검색 projection 저장소를 구축하는 시스템**이다.
...
일러스트 이미지에 객체/속성 태그를 자동 생성하고 저장해, 사용자가 객체명으로 이미지를 찾을 수 있는 검색 인덱스를 만든다.
...
4. 검색 결과는 `content_id + illustration_id + preview + tags + cms_ref`를 반환해야 한다.
5. low-confidence 태그는 운영 검수 대상으로 분리할 수 있어야 한다.
6. CMS/JSON은 원본 truth로 유지하고, DocFinder는 searchable projection만 관리해야 한다.
7. 태깅 파이프라인은 LangGraph로 명시적으로 제어 가능해야 한다.
...
4. reranker 중심 검색 품질 개선
5. 생성형 답변
6. 범용 에이전트 시스템
7. CMS 전체 데이터를 대체 저장하는 구조

핵심 원칙은 명확하다.

> **1차 목적은 태그 생성 및 저장이며, 검색은 태그 인덱스를 활용하는 수준으로 시작한다.**
...
`문제 번호만`으로 식별하는 방식은 비추천이다. 하나의 문제나 콘텐츠에 이미지가 여러 장 붙을 수 있어 충돌과 추적 문제가 생기기 쉽다.
...
- 1차 검색은 `정확 태그 + 동의어/표기 정규화`까지 포함한다.
...
- 사과가 포함된 일러스트가 반환된다.
- 검색 결과에는 `content_id`, `illustration_id`, 대표 태그, preview 정보가 포함된다.
...
### 시나리오 D. 콘텐츠 단위가 아닌 이미지 단위 식별
사용자 입력: 콘텐츠 내부 이미지 중 특정 객체 검색

기대 결과:

- `illustration_id` 기준으로 정확한 이미지가 구분된다.
- 같은 `content_id`에 여러 이미지가 있어도 혼동하지 않는다.
...
### 원칙 4. 검색의 핵심은 normalized tag다
1차 MVP의 검색 중심은 벡터 검색이 아니라 `normalized_tags`다.

- `사과`
- `apple`
- 복수형, 표기 변형

이런 차이를 정규화해 동일 개념으로 찾을 수 있어야 한다.

### 원칙 5. LangGraph는 태깅 파이프라인 제어 레이어다
LangGraph는 범용 agent가 아니라 아래 단계를 통제하기 위한 워크플로우 레이어다.
...
이 프로젝트의 핵심은 단순 모델 호출이 아니라 **대량 태깅 파이프라인을 안정적으로 제어하는 것**이다.
...
> **이 프로젝트는 검색 엔진보다 먼저 태깅 파이프라인의 안정성이 중요하다.**

따라서 LangGraph는 여전히 유효하다.
...
```json
{
  "content_id": "content_1024",
  "illustration_id": "illu_0007",
  "object_path": "cms://assets/content_1024/illu_0007.png",
  "sha256": "9b3d...",
  "file_size": 412321,
  "width": 512,
  "height": 512,
...
1차 MVP의 검색은 아래 흐름으로 고정한다.

```text
query
  → normalize query
  → match normalized_tags
  ...
  "results": [
    {
      "content_id": "content_1024",
      "illustration_id": "illu_0007",
      "thumbnail_url": "cms://thumbs/content_1024/illu_0007.png",
      "tags": ["사과", "fruit"],
      "confidence": 0.94,
      "cms_ref": {
        "content_id": "content_1024",
...
#### `GET /images/{illustration_id}/tags`
...
- 하나의 콘텐츠에 일러스트가 여러 장 있어도 `illustration_id` 기준으로 정확히 구분된다.
...
### Phase 1. 태그 인덱스 MVP
목표: 대량 이미지에 태그를 생성하고 객체명으로 찾을 수 있는 최소 기능 완성
...
- `content_id + illustration_id`를 표준 키로 고정
...
### 데이터/적재
- [ ] 이미지 자산 목록 확보
- [ ] `content_id + illustration_id` 기준 정리
- [ ] 파일 검증 규칙 구현
- [ ] 중복 방지 규칙 구현
...
1. 1차 목적은 검색보다 `일러스트 태그 생성 및 저장`이다.
2. 검색은 태그 인덱스를 활용한 객체명 검색이 중심이다.
...
6. 기본 식별 키는 `content_id + illustration_id`다.
7. LangGraph는 태깅 파이프라인 통제 레이어로 유지한다.
...
- `content_id + illustration_id`를 표준 키로 유지할 것
