...
def test_graph_prioritizes_exact_tag_hits_over_semantic_candidates() -> None:
    ...
    repository.upsert_image(
        ImageDocument(
            ...
            tag_text_projection="apple 사과 빨간 과일",
            confidence=0.96,
            review_status="approved",
            embedding=embedding_service.embed_text("apple 사과 빨간 과일"),
    ...
    repository.upsert_image(
        ImageDocument(
            ...
            normalized_tags=["fruit"],
            tag_text_projection="fruit 과일 빨간 과일 apple-like",
            confidence=0.80,
            review_status="approved",
            embedding=embedding_service.embed_text("fruit 과일 빨간 과일 apple-like"),
            tagged_at=datetime.now(UTC),
        )
    )
